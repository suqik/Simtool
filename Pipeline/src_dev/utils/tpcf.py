...
def get_cross_iso(cat1, cat2, boxsize,
    ...
    if rand1 is not None and rand2 is not None:
        rand1 = rand1.astype(dtype)
        rand2 = rand2.astype(dtype)
        IFRAND = True
        if wran1 is None:
            wran1 = np.ones(len(rand1)).astype(dtype)
        else:
            wran1 = wran1.astype(dtype)
        if wran2 is None:
            wran2 = np.ones(len(rand2)).astype(dtype)
        else:
            wran2 = wran2.astype(dtype)

    else:
        IFRAND = False

    if IFRAND:
        cat_list = [cat1, cat2, rand1, rand2]
        w_list = [wcat1, wcat2, wran1, wran2]
        label = ['A','B','X','Y']
        pair = ['AB', 'XY']
        cf = ["AB / XY - 1"]
...
### TODO: Implement faster jackknife estimation
def get_stacked_vprof(dm, void, boxsize, Rmins, Rmaxs, Rv_col=-1, rdm=None, rvoid=None, wdm=None, wv=None,
    ...
    xi_iso_stacked = np.sum(total_results['pairs']['AB'], axis=1)*total_results['normalization']['AB']/RR_ana - 1
